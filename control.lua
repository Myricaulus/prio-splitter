...
local transport_fsm = {
   ...
   -- Output lines.
   -- Prio
   [ transport_line.left_split_line ] = transport_line.right_split_line,
   [ transport_line.right_split_line ] = transport_line.left_split_line,
   -- Secondary/Overflow
   [ transport_line.secondary_left_split_line ] = transport_line.secondary_right_split_line,
   [ transport_line.secondary_right_split_line ] = transport_line.secondary_left_split_line
}

-- Belts for the splitters, used to determine the splitters' speeds.
local prio_splitter_belts = {
   ["prio-splitter"] = "transport-belt",
   ["prio-fast-splitter"] = "fast-transport-belt",
   ["prio-express-splitter"] = "express-transport-belt",

   -- Bob's Logistics mod
   ["prio-green-splitter"] = "green-transport-belt",
   ["prio-purple-splitter"] = "purple-transport-belt",
}
script.on_load (function() dprint("loaded") end)
script.on_init (function() initialize() end)
script.on_configuration_changed (
   function (data)
      local rebuild = false
      local changes = data.mod_changes
		dprint("on_configuration_changed detected")
      if changes then
      ...
      if changes and changes[modname] then
         ...
         elseif mod.old_version ~= mod.new_version then
            ...
            elseif mod.new_version >= "1.0.6" then
               ...
               -- prio splitters to the new data format.
...
script.on_event (
   ...
   function (event)
      ...
      if is_splitter (entity) then
		 ...
		 dprint (event.tick, "SPEED:", splitter.speed, splitter.tpu)
      end
   end
)
...
function initialize()
   -- The prio splitters that have been placed in the world.
...
function create_splitter (entity)
   ...
   --[[--
      The state data for a splitter. The fields are:
        id      - monotonic id
        key     - position key
        entity  - the prio splitter game object
        ...
        output  - state tracking for outbound line
		outputoverflow - state tracking for outbound overflow line
   ...
   local splitter = {
	  ...
	  outputoverflow=transport_line.secondary_left_split_line,
	  outputItemBuffer=deque.new(),
...
function process_splitter (tick, splitter, key)
   ...
   if splitter.backoff > 1 then
	--dprint("backoff red", splitter.backoff)
      return
   end
   ...
   local output_iter_overflow = transport_iter (splitter.outputoverflow)
   ...
   local outputItemBuffer = splitter.outputItemBuffer
	...
	for input in input_iter do
		...
		while item do 
			...
			if List.size(outputItemBuffer) < 8 then
				-- still space in buffer so fill it
				deque.pushright(outputItemBuffer, stack)
				dprint("Buffer:", #outputItemBuffer, List.size(outputItemBuffer), outputItemBuffer.last)
				...
				dprint("buffer full")
				-- Buffer full, so overflow immediately 
				local output = output_iter_overflow() 
				-- Nothing more to do for any input line.
				if output == nil then goto done end
				local tout = lines[output]
				local success = tout.insert_at (INSERT_POS, deque.getleft(outputItemBuffer))
				if success then
					-- Update the last lines that were successfully used.
					deque.removeleft(outputItemBuffer)
					splitter.outputoverflow = output
					
				else
					dprint("failed to insert into overflow")
					goto afterInsert
				end
			end 
		end
	...
	while List.size(outputItemBuffer) > 0 do
		
		local output = output_iter()
		local stack = deque.getleft(outputItemBuffer)
		-- Nothing more to do for any input line.
		if output == nil then goto done end
		local tout = lines[output]
		local success = tout.insert_at (INSERT_POS, stack)
		if success then
			dprint("#output", List.size(outputItemBuffer))
			-- Update the last lines that were successfully used.
			deque.removeleft(outputItemBuffer)
	...
	if inserts > 0 then
		...
		dprint("___backoff", splitter.backoff)
	end
end
...
function is_splitter (entity)
   -- Or more specifically, a prio splitter.
   if entity.type ~= "splitter" then
      return false
   else
      -- And do we support it.
      return prio_splitter_belts[entity.name]
   end
end


function splitter_belt_speed (splitter_name)
   local belt_name = prio_splitter_belts[splitter_name]
